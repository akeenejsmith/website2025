...
// Then your existing jQuery code continues as normal:
jQuery(function ($) {
    // Remove the duplicate session storage code you added earlier
		...
		if( $('.projects-list-wrapper').length > 0 ){
			...
			if (!$("body").hasClass("disable-ajaxload")) {
				$('.projects-list-captions li span').on('click', function() {
					...
					// Make the "Menu" text clickable on mobile
jQuery(document).ready(function($) {
    
    // Find the menu text element and make it trigger the burger menu
    $('.button-wrap.menu .button-text').on('click touchstart', function(e) {
        e.preventDefault();
        e.stopPropagation();
        
        // Trigger the same click event that the burger button uses
        var $burgerButton = $(this).siblings('.icon-wrap').find('#burger-wrapper');
        var $menuBurger = $('#menu-burger');
        var $buttonWrap = $('.button-wrap.menu');
        
        // Toggle the menu classes (same as the original burger functionality)
        ...
        $('.clapat-nav-wrapper').toggleClass('open');
    });
    
    // Also make sure the text has a pointer cursor on mobile
    $('.button-wrap.menu .button-text').css('cursor', 'pointer');
	...
	// Simple Mobile Resume Redirect
function setupMobileResume() {
    // Check if it's a mobile device
    const isMobile = /Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent) || window.innerWidth <= 470;
    
    // Find the resume button and update it for mobile
    const resumeButton = document.querySelector('#resumedownload a');
    if (resumeButton && isMobile) {
        resumeButton.href = 'https://akeenejsmith.framer.website/#mobile-resume';
    }
}

// Run when page loads
document.addEventListener('DOMContentLoaded', setupMobileResume);
...
// Fix for portrait mode button visibility
window.addEventListener('orientationchange', function() {
    setTimeout(function() {
        ScrollTrigger.refresh();
        // Force button visibility
        gsap.set('.button-box, .clapat-button-wrap, .button-border', {
            opacity: 1,
            visibility: 'visible',
            y: 0
        });
	...
	// Enhanced large mobile detection
function redirectLargeMobile() {
    ...
    // Large mobile screens (iPhone 12 Pro Max and up, large Android phones)
    const isLargeMobile = isMobile && (
        screenHeight >= 926 || // iPhone 12 Pro Max height or larger
        screenWidth >= 428     // iPhone 12 Pro Max width or larger
    );
    
    if (isLargeMobile) {
        window.location.href = 'https://akeenejsmith.framer.website';
    }
}

document.addEventListener('DOMContentLoaded', redirectLargeMobile);
